...
It validates six observable behaviors per scenario:
...
- **`observability.tool.order`** — every declared happens-before constraint holds: each call to an `after` tool is preceded by its `before` tool (e.g. authorize before charging).
...
- `expectedToolOrder` — happens-before tool constraints (`before`/`after` pairs) the tool calls must satisfy.
